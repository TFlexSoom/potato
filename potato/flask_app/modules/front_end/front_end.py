...
class TemplateConfig:
    ...
    base_html_template: str = HTML_TEMPLATE_DICT["base_html_template"]["default"]
    header_file: str = HTML_TEMPLATE_DICT["header_file"]["default"]
    surveyflow_html_layout: str = HTML_TEMPLATE_DICT["surveyflow_html_layout"]["default"]
    site_dir: str = ""
    config_file: str = ""
    jumping_to_id_disabled: bool = False
    hide_navbar: bool = False
    annotation_schemes: str = ""
...
def generate_task_html_and_bindings(abs_html_layout_path):
    ...
    for schema_name, schema_layout in schema_layouts.items():
...
def compile_data(paths: TemplatePaths, templates: Template):
    ...
    if len(annotation_codebook_url) > 0:
        codebook_html = (
            ...
            ' class="nav-item nav-link">Annotation Codebook</a>'
        )
...
def generate_site(paths: TemplatePaths):
    """
    ...
    html_template: Template = templates.base_html_template
    html_template = correct_template(html_template)
    html_template = compile_template(html_template, compile_data())
    
    site_name = f"{hyphenated_task_name}-{os.path.basename(paths.base_html_template)}"
